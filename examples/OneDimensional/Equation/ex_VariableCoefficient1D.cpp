// ============================================================================
// Arquivo: ex_VariableCoefficient1D.cpp
// Projeto: FVMaker
// Versão: consulte <FVMaker/Core/Version.h>
// Descrição: Implementa ex Variable Coefficient 1 D no contexto de projeto.
...
[[nodiscard]] fvm::Real gamma_coefficient(fvm::Real x) {
    return fvm::Real{1.0} + x;
}

[[nodiscard]] fvm::Real source(fvm::Real x) {
    return pi * std::cos(pi * x)
        - (fvm::Real{1.0} + x) * pi * pi * std::sin(pi * x);
}
...
int main() {
    ...
    const fvm::Equation1D equation{
        grid,
        fvm::Laplacian1D{fvm::function_coefficient_1d(grid, gamma_coefficient)},
    ...
    std::cout << "Variable coefficient 1D\n";
    std::cout << "d/dx(gamma(x) dphi/dx) = f(x)\n";
    std::cout << "gamma(x) = 1 + x\n";
    std::cout << "=================================\n";
    ...
    for (fvm::Size i = 0; i < grid.num_volumes(); ++i) {
        std::cout << std::setw(5) << i
                  ...
                  << std::setw(16) << gamma_coefficient(grid.faces()[i])
                  << std::setw(16) << result.solution[i]
                  << std::setw(16) << phi_exact(grid.centers()[i])
                  << '\n';
    }

    return result.converged && fvm::norm_infinity(error) < fvm::Real{1.0e-3}
        ? 0
        : 1;
}
